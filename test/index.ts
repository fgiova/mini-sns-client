...
test("MiniSNSClient", { only: true }, async (t) => {
	...
	await t.test("publishMessage Using signer instance", async (t) => {
		...
		const client = new MiniSNSClient(
			"eu-central-1",
			undefined,
			{
				factory: () => mockPool,
  ...
  <ResponseMetadata>
    <RequestId>93313591-271d-50b4-a0ca-685d2fb07219</RequestId>
		...
		t.same(
			...
			messagBatch.PublishBatchRequestEntries?.[0].Id,
		);
	});
  ...
  <ResponseMetadata>
    <RequestId>93313591-271d-50b4-a0ca-685d2fb07219</RequestId>
		...
		t.same(
			result.Successful[1].Id,
			messagBatch.PublishBatchRequestEntries?.[1].Id,
		);
	});
	await t.test("publishMessage batch multiple item > 10", async (t) => {
		const { mockPool, client } = t.context;
		function splitArray<T>(items: T[]): T[][] {
		...
		mockPool
			.intercept({
				...
				body: (body: string) => {
					...
					const messagesChunksIdsSorted = responsesChunks[0]
						.map((item) => item.Id)
						.sort();
					return (
						JSON.stringify(entriesIdsSorted) ===
						JSON.stringify(messagesChunksIdsSorted)
					);
				},
			})
			.reply(
  ...
  <PublishBatchResult>
    <Failed/>
    <Successful>${responsesChunks[0].map(
			(chunk, _index) =>
				`<member>
        <MessageId>${randomUUID()}</MessageId>
        <Id>${chunk.Id}</Id>
		...
		mockPool
			.intercept({
				...
				body: (body: string) => {
					...
					const entriesIds = entries.map((item) => item.split("=")[1]);
					const entriesIdsSorted = entriesIds.sort();
					const messagesChunksIdsSorted = responsesChunks[1]
						.map((item) => item.Id)
						.sort();
					return (
						JSON.stringify(entriesIdsSorted) ===
  ...
  <PublishBatchResult>
    <Failed/>
    <Successful>${responsesChunks[1].map(
			(chunk, _index) =>
				`<member>
        <MessageId>${randomUUID()}</MessageId>
        <Id>${chunk.Id}</Id>
  ...
  <ResponseMetadata>
	...
	await t.test(
		"publishMessage batch multiple item w a failed message",
		async (t) => {
			const { mockPool, client } = t.context;
			const messagBatch = {
				PublishBatchRequestEntries: [
